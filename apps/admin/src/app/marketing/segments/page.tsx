...
export default async function SegmentsPage() {
  ...
  const { data: segments } = await supabase.from('marketing_segments').select('*').order('created_at', { ascending: false });

  return (
    <div className="p-8 max-w-5xl mx-auto space-y-8">
      <div>
         <h1 className="text-3xl font-bold text-slate-900">Marketing Segments</h1>
      ...
      <div className="grid md:grid-cols-3 gap-8">
         ...
         <div className="md:col-span-2 space-y-4">
            ...
            {segments?.map((segment: any) => (
                <div key={segment.id} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
