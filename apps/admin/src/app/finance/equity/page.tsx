                            ...
                            <div key={round.id} className="flex justify-between items-center border-b pb-2 last:border-0">
                                <div>
                                    ...
                                    <div className="text-sm text-muted-foreground">
                                        {new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(round.price_per_share)} / share
                                    </div>
                                </div>
                                <div className="text-right">
                                    <div className="text-sm text-muted-foreground">Valuation Cap</div>
                                    <div className="font-bold">
                                        {new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', notation: 'compact' }).format(round.valuation_cap)}
                                    </div>
                                </div>
                            </div>
                        ))}
                         {rounds.length === 0 && <div className="text-muted-foreground text-sm">No rounds found.</div>}
                    </div>
                </Card>
            </div>
